...
//}
#define IMPL_ROLE(role) \
  ROLE_PROTO_TYPE(role) { return const_cast<role &>(static_cast<const role &>(*this)); }

//////////////////////////////////////////////////////////////////
#define IMPL_ROLE_WITH_TYPE(role, type) \
  ROLE_PROTO_TYPE(role) override { return const_cast<type &>(static_cast<const type &>(*this)); }
...
#define IMPL_ROLE_WITH_VAR(role, type)                                            \
 private:                                                                         \
  ROLE_PROTO_TYPE(role) override { return const_cast<type &>(__TYPE_VAR(type)); } \
...
#define IMPL_ROLE_WITH_ROLE_VAR(role)                                      \
 private:                                                                  \
  ROLE_PROTO_TYPE(role) override { return const_cast<role &>(role##var); } \
...
#define DECL_DEPENDENCY(RoleType)                                               \
 public:                                                                        \
  void SET_ROLE(RoleType)(RoleType & role) { ROLE_PTR(RoleType) = &role; }      \
  DECL_ROLE(RoleType) { return *(const_cast<RoleType *>(ROLE_PTR(RoleType))); } \
                                                                                \
 private:                                                                       \
