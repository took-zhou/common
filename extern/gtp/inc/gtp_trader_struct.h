#ifndef _GTP_TRADER_STRUCT_TYPE_H_
#define _GTP_TRADER_STRUCT_TYPE_H_
...
struct GtpTransactionCostField {
  char exchange_id[10];
  char instrument_id[32];
  char user_id[10];
};

struct GtpCommissionInfo {
  char exchange_id[10];
  char instrument_id[32];
  char user_id[10];
  int32_t prid;
