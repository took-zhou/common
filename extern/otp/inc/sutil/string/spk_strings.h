 ...
 * @param       pStr            待拷贝的字符串
 * @param       maxStrLen       最大有效字符长度(不包括结尾的'\0'), 即buf的长度需至少为 maxStrLen + 1
...
SStr_StrCopy2(char *pBuf, const char *pStr, int32 maxStrLen,
        int32 *pOutCopyLen) {
    const int32         theMaxLen = maxStrLen >= 0 ? maxStrLen : INT_MAX;
    int32               n = 0;
...
SStr_StrFixedLengthTrimCopy(char *pBuf, const char *pStr, int32 maxStrLen) {
    ...
    while (maxStrLen != 0 && SPK_ISSPACE(*pStr)) {
        pStr++;
        maxStrLen--;
    }

    while (n < maxStrLen && pStr[n]) {
        pBuf[n] = pStr[n];
        n++;
    }
...
#   pragma GCC diagnostic ignored "-Wstringop-overflow"
    ...
    SLOG_ASSERT(n >= 0 && (n <= maxStrLen || maxStrLen < 0));
    do {
        pBuf[n--] = '\0';
    } while (n >= 0 && SPK_ISSPACE(pBuf[n]));
