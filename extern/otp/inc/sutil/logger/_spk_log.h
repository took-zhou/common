...
#   define  _SPK_VA_MACRO_ABLE          (0)
#else
#   define  _SPK_VA_MACRO_ABLE          (1)
#endif
 ...
 */
#if defined (NO_LOG)
#   define  _SPK_LOG_ABLE               (0)
#else
#   define  _SPK_LOG_ABLE               (1)
#endif

/**
 * 是否允许登记级别为TRACE的日志信息
 */
#if defined (NO_TRACE) || ! _SPK_LOG_ABLE
#   define  _SPK_LOG_TRACE_ABLE         (0)
#else
#   define  _SPK_LOG_TRACE_ABLE         (1)
#endif
/* -------------------------           */
 ...
 */
#define SPK_DEFAULT_LOG_MASK            (0x0)
#define SPK_DEFAULT_NO_VA_LOG_MASK      SPK_DEFAULT_LOG_MASK
/* -------------------------           */
...
#else
#   undef   _SLOG_MASK
#   define  _SLOG_MASK                  SPK_DEFAULT_LOG_MASK
#endif
...
#   undef   SLOG_ASSERT
#   undef   SLOG_ASSERT2
#   undef   SLOG_ASSERT_NA
#   undef   SLOG_ASSERT_NA2
...
#       define  SLOG_LOG(...)       \
        ...
        /**
         * SLOG_TRACE2, 跟踪信息登记宏函数
         * 以级别LOG_TRACE登记日志 (不会被 NO_TRACE 预编译参数禁用)
         *
         * <pre>
         * SLOG_TRACE2("log info");
         * SLOG_TRACE2("log info, a is:%s; b is: %d", a, b);
         * </pre>
         */
#       define  SLOG_TRACE2(...)    \
                _SLog_LogImpl(__FILE__, sizeof(__FILE__), __LINE__, \
...
#else       /* _SPK_VA_MACRO_ABLE */
#   if _SPK_LOG_ABLE
#       ifdef _SLOG_MASK
#           define      SLOG_LOG        _SLog_LogFullSimplenessMasked
#           define      SLOG_TRACE2     _SLog_TraceSimplenessMasked
...
#           define      SLOG_TRACE2     _SLog_TraceSimpleness
...
#if !defined(NDEBUG) && !defined(SPK_ASSERT_NA_ASSERT_ABLE)
/**
 * 类 assert 接口, 但仅记录日志, 不触发 assert
 */
#   define  SLOG_ASSERT_NA(EXPR)                    \
        do { \
            if (__spk_unlikely(! (EXPR) )) { \
                SLOG_ERROR("[ERROR] ASSERT_NA: (" __STRING(EXPR) ")"); \
            } \
        } while (0)
...
# else  /* _SPK_VA_MACRO_ABLE */
#   define  SLOG_ASSERT_NA2             _SLog_AssertSimplenessNA
# endif


#elif !defined(NDEBUG)
# if _SPK_VA_MACRO_ABLE
#   define  SLOG_ASSERT_NA(EXPR)        SLOG_ASSERT((EXPR))
#   define  SLOG_ASSERT_NA2(EXPR, ...)  SLOG_ASSERT2((EXPR), __VA_ARGS__)
# else  /* _SPK_VA_MACRO_ABLE */
#   define  SLOG_ASSERT_NA              SLOG_ASSERT
#   define  SLOG_ASSERT_NA2             SLOG_ASSERT2
# endif
