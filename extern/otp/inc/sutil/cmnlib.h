...
#undef  SPK_MIN3
#define SPK_MIN3(a,b,c)                 ( SPK_MIN(SPK_MIN((a), (b)), (c)) )
...
#undef  SPK_MAX3
#define SPK_MAX3(a,b,c)                 ( SPK_MAX(SPK_MAX((a), (b)), (c)) )
 ...
 * 对浮点数进行 == 和 != 比较操作的辅助宏函数
 */
#define SPK_IS_FLOAT_EQ(f1, f2)                         \
        ( (f1) < (f2) + SPK_FLOAT_WINDAGE && (f1) > (f2) - SPK_FLOAT_WINDAGE )

#define SPK_IS_FLOAT_NE(f1, f2)                         \
        ( (f1) > (f2) + SPK_FLOAT_WINDAGE || (f1) < (f2) - SPK_FLOAT_WINDAGE )

#define SPK_IS_DOUBLE_EQ(f1, f2)                        \
        ( (f1) < (f2) + SPK_DOUBLE_WINDAGE && (f1) > (f2) - SPK_DOUBLE_WINDAGE )

#define SPK_IS_DOUBLE_NE(f1, f2)                        \
        ( (f1) > (f2) + SPK_DOUBLE_WINDAGE || (f1) < (f2) - SPK_DOUBLE_WINDAGE )
/* -------------------------           */
 ...
 * 若c为非可打印字符则返回v
 */
#undef  SPK_IF_NONPRITABLE
#define SPK_IF_NONPRITABLE(c,v)         (! SPK_ISPRINT(c) ? (v) : (c))
