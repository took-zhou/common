 ...
 * e.g. SPK_ROUND_TIMES(1001, 100) => 11
 */
 ...
 * e.g. SPK_ADJUST_SIZE(1001, 100) => 1100
 */
...
/*
 * 返回 SIZE / BASESIZE 的整除倍数
 * e.g. SPK_FLOOR_TIMES(1099, 100) => 10
 */
#undef  SPK_FLOOR_TIMES
#define SPK_FLOOR_TIMES(SIZE, BASESIZE)                 \
        ( (SIZE) / (BASESIZE) )

/*
 * 返回按BASESIZE的向下舍入后的SIZE
 * e.g. SPK_FLOOR_SIZE(1099, 100) => 1000
 */
#undef  SPK_FLOOR_SIZE
#define SPK_FLOOR_SIZE(SIZE, BASESIZE)                  \
        ( SPK_FLOOR_TIMES((SIZE), (BASESIZE)) * (BASESIZE) )
/* -------------------------           */
...
/* 返回按512字节对齐的SIZE */
#undef  SPK_ALIGN_SIZE512
#define SPK_ALIGN_SIZE512(SIZE)         SPK_ALIGN_SIZE((SIZE), 512)
...
#   define  SPK_TIMED_SLEEP_SINCE(MS, P_SINCE_TV)       \
    do { \
        ...
        long            __TIMED_SINCE_msec = (MS); \
        __spk_gettimeofday(&__TIMED_SINCE_now, (STimezoneT *) NULL); \
        __TIMED_SINCE_diff = \
