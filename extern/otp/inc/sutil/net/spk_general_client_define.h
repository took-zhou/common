 ...
 * 适用于一般情况下的通用客户端环境
 *
 * @version 0.8.12      2016/8/10
 * @version 0.15.9.11   2020/01/07
 *          - 扩大通道组和通信端点的最大连接数量到256:
 *              - GENERAL_CLI_MAX_CHANNEL_GROUP_SIZE 的取值由 128 调整为 256
 * @version 0.17.6.7    2022/11/22
 *          - 为支持将登录密码配置为加密后的密文密码, 而扩展配置结构 (SGeneralClientAddrInfoT, SGeneralClientRemoteCfgT) 中 password 字段的长度:
...
/** 最大的心跳间隔(秒) */
#define GENERAL_CLI_MAX_HEARTBEAT_INTERVAL              (900)
...
/** 连接通道组的最大连接数量 */
#ifndef GENERAL_CLI_MAX_CHANNEL_GROUP_SIZE
...
/** BASE64编码后的密码最大长度 */
#define GENERAL_CLI_MAX_PWD_BASE64_LEN                  (128)
/* -------------------------           */
...
typedef struct _SGeneralClientChannel {
    ...
    SDataBufferT        __codecBuf;         /**< 编解码缓存 */
    SDataBufferT        __recvBuf;          /**< 接收缓存 */
...
typedef struct _SGeneralClientAddrInfo {
    ...
    /** 密码 */
    char                password[GENERAL_CLI_MAX_PWD_BASE64_LEN];
