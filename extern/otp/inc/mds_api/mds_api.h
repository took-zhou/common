 ...
 * @version 0.17.6.7    2022/11/20
 *          - 为支持将登录密码配置为加密后的密文密码, 而添加如下接口:
...
#define MDSAPI_DEFAULT_TICK_RESEND_TIMEOUT_MS   (30 * 1000)
/** 逐笔数据重传的最大超时时间 (长时间请求不到任何数据时的超时时间, 单位:毫秒) */
#define MDSAPI_MAX_TICK_RESEND_TIMEOUT_MS       (900 * 1000)
 ...
 * - MDS_MSGTYPE_L2_TICK_CHANNEL_HEARTBEAT          => @see MdsTickResendRequestRspT
 * </p>
 *
 ...
 * @exception   ENOTSUP         版本不兼容或加密方法等不支持
 * @exception   EACCES          用户名或密码错误
 * @exception   EMLINK          连接数量超过限制
 * @exception   ESRCH           没有找到主节点
 ...
 *              逐笔委托数据, 需要使用 MdsApi_SendTickResendRequest2 接口
 *
 ...
 * @retval      <0              失败 (负的错误号)
 */
int32   MdsApi_SendTickResendRequest(
                MdsApiSessionInfoT *pQryChannel,
                uint8 exchangeId,
                uint16 channelNo,
 ...
 * @retval      <0              失败 (负的错误号)
 */
int32   MdsApi_SendTickResendRequest2(
                MdsApiSessionInfoT *pQryChannel,
...
/**
 * 发送超大的逐笔数据重传请求
 * 支持不限制大小的重传请求 (将自动拆分为多条小的逐笔数据重传请求发送到MDS服务器)
 *
 ...
 * @retval      <0              失败 (负的错误号)
 */
int32   MdsApi_SendTickResendRequestHugely(
                MdsApiSessionInfoT *pQryChannel,
                const MdsTickResendRequestReqT *pResendReq,
                F_MDSAPI_ON_QRY_MSG_T fnQryMsgCallback,
                void *pCallbackParams,
...
/**
 * 设置客户端本地安全密钥 (对当前进程生效, 用于解密密文密码)
 *
 * @param   pSecretKey          本地安全密钥 (用于解密密文密码)
 */
void    MdsApi_SetLocalSecretKey(
                const char *pSecretKey);

/**
 * 重置(清空)客户端本地安全密钥 (对当前进程生效)
 */
void    MdsApi_ResetLocalSecretKey();
 ...
 * 不设置或者参数为空的话, 登录时会尝试使用配置文件中的配置
 *
 * @note    推荐通过设置 MdsApiRemoteCfgT.password 的方式在连接级别单独设置, 以避免多个
 *          连接之间相互干扰, 例如:
 *          - MdsApiRemoteCfgT        tcpChannelCfg;
 *          - strncpy(tcpChannelCfg.password, "password", sizeof(tcpChannelCfg.password) - 1);
 *
 * @param   pPassword           登录密码
 *                              - 支持通过密码前缀指定密码类型, 如 md5:PASSWORD, txt:PASSWORD
 * @return  TRUE 设置成功; FALSE 设置失败 (无效的密码字符串或密码前缀)
 */
BOOL    MdsApi_SetThreadPassword(
                const char *pPassword);

/**
 * 返回当前线程登录MDS时使用的登录密码的加密类型
 * (即通过 MdsApi_SetThreadPassword 方法设置的登录密码的加密类型)
 *
 * @return  当前线程登录MDS时使用的登录密码的加密类型
 * @see     eMdsExtClientEncryptTypeT
 * @see     eGeneralClientEncryptTypeT
 */
int32   MdsApi_GetThreadPasswordEncryptType();
...
/**
 * 设置SubscribeByString接口默认使用的逐笔数据的过期时间类型
 *
 * @param   tickExpireType      逐笔数据的过期时间类型 @see eMdsSubscribedTickExpireTypeT
 *
 * @deprecated 已废弃, 不再支持逐笔数据的过期时间参数, 将忽略该函数调用, 不再生效
 */
...
/**
 * 生成加密后的密文密码字符串 (以提高本地存储或配置的登录密码的安全性)
 *
 * 函数说明:
 * - 该函数会通过输出缓存(pOutBuf)返回经DES加密的BASE64编码的密文密码字符串, 形如:
 *   des:xxxxx
 * - 可以将该密文字符串作为登录密码进行存储或配置, 例如:
 *   password = des:xxxxx
 *
 * @param[out]      pOutBuf             用于输出解码后密钥的缓存区
 * @param           bufSize             缓存区大小 (@note 至少 128 字节)
 * @param           pUsername           用户名 (不可为空)
 * @param           pPassword           待加密的密码字符串 (不可为空)
 * @param           pLocalSecretKey     本地安全密钥 (用于解密密文密码)
 * @return          大于等于0, 编码后的密文密码字符串长度 (包含前缀在内的总长度);
 *                  小于0, 负的错误号
 */
int32   MdsHelper_GenerateCiphertextPassword(
                char *pOutBuf,
                int32 bufSize,
                const char *pUsername,
                const char *pPassword,
                const char *pLocalSecretKey);
/* -------------------------           */
