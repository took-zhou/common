 ...
 *          - 有效竞价范围基准价类型(eOesAuctionReferPriceTypeT) 中:
 *              - 新增 买一卖一价(OES_AUCTION_REFER_PRICE_TYPE_SSE_BEST) 定义, 仅适用于上交所
 * @version 0.17.6.8    2023/02/10
 ...
 * @version 0.17.6.9    2023/02/XX
 *          - 证券状态枚举(eOesSecurityStatusT) 中:
...
/** 个股持仓比例阀值百分比单位 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
...
typedef enum _eOesSecurityStatus {
    ...
    OES_SECURITY_STATUS_FIRST_DELIST        = (1 << 10) /**< 退市整理期首日 */
} eOesSecurityStatusT;
...
typedef enum _eOesAuctionLimitType {
    OES_AUCTION_LIMIT_TYPE_NONE             = 0,        /**< 无申报价格范围限制 */
 ...
 *      4. OES_ORD_TYPE_MTL_BEST
 *      5. OES_ORD_TYPE_MTL_SAMEPARTY_BEST
 *
 * 上海期权支持市价类型:
 *      1. OES_ORD_TYPE_LMT
