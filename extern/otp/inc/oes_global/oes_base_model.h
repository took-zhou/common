 ...
 * @version 0.17.6.8    2023/02/xx
 *          - 为支持全面注册制改革, '现货产品基础信息 (OesStockBaseInfoT)' 中:
 ...
 *          - 为支持全面注册制改革, '证券发行基础信息(OesIssueBaseInfoT)' 中:
 *              - 调整 isWeightedVotingRights 字段说明, 由 '是否存在投票权差异' 改为 '是否存在表决权差异'
 *
 * @since   2015/07/30
 */
...
#define __OES_FUND_TRSF_BASE_INFO_PKT                                   \
        ...
        union { \
            ...
            uint8       fundTrsfType; \
            /** 出入金转账类型 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            uint8       isAllotOnly; \
        }; \
        /** 按64位对齐的填充域 */ \
...
#define __OES_INV_ACCT_BASE_INFO_PKT                                    \
        ...
        /** 席位号 */ \
        int32               pbuId; \
        /** 个股持仓比例阀值 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
...
typedef struct _OesPriceLimit {
    union {
        ...
        /** 涨停价 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
        int32           ceilPrice;
    };
    ...
    union {
        ...
        /** 跌停价 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
        int32           floorPrice;
    };
} OesPriceLimitT;
...
#define __OES_STOCK_BASE_INFO_PKT                                       \
        ...
        /** 是否尚未盈利 (0:已盈利, 1:未盈利) */ \
        uint8               isNoProfit; \
        /** 是否存在表决权差异 (0:无差异, 1:有差异) */ \
        uint8               isWeightedVotingRights; \
        /** 是否具有协议控制架构 (0:没有协议控制架构, 1:具有协议控制架构. 仅适用于深交所, 对于上交所该字段固定为0) */ \
        uint8               isVie; \
        ...
        union { \
            ...
            int32           priceTick; \
            /** 最小报价单位 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int32           priceUnit; \
        }; \
        ...
        union { \
            /** 单笔限价买委托数量上限 */ \
            int32           lmtBuyMaxQty; \
            /** 单笔限价买委托数量上限 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int32           buyOrdMaxQty; \
        }; \
        union { \
            /** 单笔限价买委托数量下限 */ \
            int32           lmtBuyMinQty; \
            /** 单笔限价买委托数量下限 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int32           buyOrdMinQty; \
        }; \
        union { \
            /** 单笔限价买入单位 */ \
            int32           lmtBuyQtyUnit; \
            /** 单笔限价买入单位 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int32           buyQtyUnit; \
        }; \
        /** 单笔市价买委托数量上限 */ \
        int32               mktBuyMaxQty; \
        ...
        union { \
            /** 单笔限价卖委托数量上限 */ \
            int32           lmtSellMaxQty; \
            /** 单笔限价卖委托数量上限 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int32           sellOrdMaxQty; \
        }; \
        union { \
            /** 单笔限价卖委托数量下限 */ \
            int32           lmtSellMinQty; \
            /** 单笔限价卖委托数量下限 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int32           sellOrdMinQty; \
        }; \
        union { \
            /** 单笔限价卖出单位 */ \
            int32           lmtSellQtyUnit; \
            /** 单笔限价卖出单位 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int32           sellQtyUnit; \
        }; \
        /** 单笔市价卖委托数量上限 */ \
        int32               mktSellMaxQty; \
        ...
        union { \
            ...
            int64           parValue; \
            /** 面值, 单位精确到元后四位, 即1元 = 10000 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int64           parPrice; \
        }; \
        /** 逆回购期限 */ \
        ...
        int32               cashHoldDays; \
        \
        /** 连续交易时段的有效申报价格范围限制类型 @see eOesAuctionLimitTypeT */ \
        uint8               auctionLimitType; \
        /** 连续交易时段的有效申报价格范围基准价类型 @see eOesAuctionReferPriceTypeT */ \
        uint8               auctionReferPriceType; \
        /** 按64位对齐的填充域 */ \
        uint8               __STOCK_BASE_filler1[2]; \
        /** 连续交易时段的有效申报价格范围涨跌幅度 (百分比或绝对价格, 取决于'有效申报价格范围限制类型') */ \
...
#define __OES_ISSUE_BASE_INFO_PKT                                       \
        ...
        /** 是否注册制 (0:核准制, 1:注册制) */ \
        uint8               isRegistration; \
        /** 是否尚未盈利 (0:已盈利, 1:未盈利. 仅适用于深交所, 对于上交所该字段固定为0) */ \
        uint8               isNoProfit; \
        /** 是否存在表决权差异 (0:无差异, 1:有差异. 仅适用于深交所, 对于上交所该字段固定为0) */ \
        uint8               isWeightedVotingRights; \
        /** 是否具有协议控制框架 (0:没有协议控制架构, 1:具有协议控制架构. 仅适用于深交所, 对于上交所该字段固定为0) */ \
        uint8               isVie; \
        /** 按64位对齐的填充域 */ \
        ...
        union { \
            ...
            int32           upperLimitPrice; \
            /** 申购价格上限 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int32           ceilPrice; \
        }; \
        union { \
            /** 申购价格下限 (单位精确到元后四位, 即1元 = 10000) */ \
            int32           lowerLimitPrice; \
            /** 申购价格下限 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int32           floorPrice; \
        }; \
        \
        /** 委托最大份数 */ \
        ...
        /** 总发行量 */ \
        int64               issueQty; \
        /** 配股股权登记日 (仅适用于上交所, 对于深交所该字段固定为0) */ \
        int32               alotRecordDay; \
        /** 配股股权除权日 (仅适用于上交所, 对于深交所该字段固定为0) */ \
...
#define __OES_ETF_COMPONENT_BASE_INFO_PKT                               \
        ...
        union { \
            ...
            int32           premiumRatio; \
            /** 申购溢价比例 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
            int32           premiumRate; \
        }; \
        ...
        union { \
            ...
            /** 赎回替代金额 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */ \
