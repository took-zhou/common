 ...
 *          - 查询客户单证券融资融券负债统计信息过滤条件 (OesQryCrdSecurityDebtStatsFilterT) 中:
 *              - 新增查询条件 是否有融资融券负债标识(hasCreditDebtFlag)
 *          - 查询融资融券可充抵保证金证券及融资融券标的信息过滤条件过滤条件 (OesQryCrdUnderlyingInfoFilterT) 中:
 *              - 删除查询条件 证券账户代码(invAcctId)
 *          - 查询融资融券可充抵保证金证券及融资融券标的信息应答内容 (OesCrdUnderlyingInfoItemT) 中:
 *              - 删除 证券账户代码(invAcctId) 字段
 * @version 0.17.1.1    2021/06/24
 *          - 重命名头文件 oes_qry_credit_packets.h => oes_qry_packets_credit.h
 *          - 新增 '查询融资融券业务可取资金' 相关报文定义
 *              - 新增结构体 融资融券业务可取资金信息(OesCrdDrawableBalanceItemT)
 *              - 新增结构体 查询融资融券业务可取资金信息请求(OesQryCrdDrawableBalanceReqT)
 *              - 新增结构体 查询融资融券业务可取资金信息应答(OesQryCrdDrawableBalanceRspT)
 *          - 新增 '查询融资融券担保品可转出的最大数量信息' 相关报文定义
 *              - 新增结构体 融资融券担保品可转出的最大数量信息(OesCrdCollateralTransferOutMaxQtyItemT)
 *              - 新增结构体 查询担保品可转出的最大数量信息请求(OesQryCrdCollateralTransferOutMaxQtyReqT)
 *              - 新增结构体 查询担保品可转出的最大数量信息应答(OesQryCrdCollateralTransferOutMaxQtyRspT)
 * @version 0.17.6.4    2022/09/21
 *          - 资金头寸信息内容(OesCrdCashPositionItemT) 中:
 *              - 新增 总计调整金额(totalAdjustAmt) 字段, 取值可以为负数, 包含红冲蓝补金额及主柜划入金额
 *          - 融资融券可充抵保证金证券及融资融券标的信息内容(OesCrdUnderlyingInfoItemT) 中:
...
#define OES_MAX_CRD_DEBT_CONTRACT_ITEM_CNT_PER_PACK         (100)

/** 每条查询应答报文中融资融券合约流水信息的最大数量 */
#define OES_MAX_CRD_DEBT_JOURNAL_ITEM_CNT_PER_PACK          (100)

/** 每条查询应答报文中信用资产信息的最大数量 */
#define OES_MAX_CRD_CREDIT_ASSET_ITEM_CNT_PER_PACK          (100)

/** 每条查询应答报文中客户单证券融资融券负债统计信息的最大数量 */
#define OES_MAX_CRD_SECURITY_DEBT_STATS_ITEM_CNT_PER_PACK   (100)

/** 每条查询应答报文中直接还款信息的最大数量 */
#define OES_MAX_CRD_CASH_REPAY_ITEM_CNT_PER_PACK            (100)

/** 每条查询应答报文中资金头寸信息 (可融资头寸信息) 的最大数量 */
#define OES_MAX_CRD_CASH_POSITION_ITEM_CNT_PER_PACK         (100)

/** 每条查询应答报文中证券头寸信息 (可融券头寸信息) 的最大数量 */
#define OES_MAX_CRD_SECURITY_POSITION_ITEM_CNT_PER_PACK     (100)

/** 每条查询应答报文中余券信息的最大数量 */
#define OES_MAX_CRD_EXCESS_STOCK_ITEM_CNT_PER_PACK          (100)

/** 每条查询应答报文中融资融券息费利率的最大数量 */
#define OES_MAX_CRD_INTEREST_RATE_ITEM_CNT_PER_PACK         (100)

/** 每条查询应答报文中融资融券可充抵保证金证券及融资融券标的信息的最大数量 */
#define OES_MAX_CRD_UNDERLYING_INFO_ITEM_CNT_PER_PACK       (500)
/* -------------------------           */
...
typedef struct _OesQryCrdCreditAssetRsp {
    ...
    OesCrdCreditAssetItemT
                        qryItems[OES_MAX_CRD_CREDIT_ASSET_ITEM_CNT_PER_PACK];
...
/* ===================================================================
 * 查询融资融券可充抵保证金证券及融资融券标的信息相关结构体定义
 * =================================================================== */

/**
 * 查询融资融券可充抵保证金证券及融资融券标的信息过滤条件
 */
typedef struct _OesQryCrdUnderlyingInfoFilter {
    /** 客户代码, 可选项 */
    char                custId[OES_CUST_ID_MAX_LEN];
    /** 证券代码, 可选项 */
    ...
    uint8               crdMarginTradeUnderlyingFlag;
    /** 是否为融券标的 (0:未指定, 1:是融券标的, 2:不是融券标的) */
    uint8               crdShortSellUnderlyingFlag;
    /** 按64位对齐的填充域 */
    uint8               __filler[5];

    /** 用户私有信息 (由客户端自定义填充, 并在应答数据中原样返回) */
    int64               userInfo;
} OesQryCrdUnderlyingInfoFilterT;


/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_CRD_UNDERLYING_INFO_FILTER                      \
        {0}, {0}, \
        0, 0, 0, {0}, \
        0
/* -------------------------           */
...
/**
 * 融资融券可充抵保证金证券及融资融券标的信息内容
 */
typedef OesCrdUnderlyingBaseInfoT   OesCrdUnderlyingInfoItemT;


/* 结构体的初始化值定义 */
#define NULLOBJ_OES_CRD_UNDERLYING_INFO_ITEM                            \
        NULLOBJ_OES_CRD_UNDERLYING_BASE_INFO
/* -------------------------           */
 ...
 */
typedef struct _OesQryCrdUnderlyingInfoReq {
    /** 查询请求消息头 */
    OesQryReqHeadT      reqHead;
    /** 查询过滤条件 */
    OesQryCrdUnderlyingInfoFilterT
                        qryFilter;
} OesQryCrdUnderlyingInfoReqT;


/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_CRD_UNDERLYING_INFO_REQ                         \
        {NULLOBJ_OES_QRY_REQ_HEAD}, \
        {NULLOBJ_OES_QRY_CRD_UNDERLYING_INFO_FILTER}
/* -------------------------           */
 ...
 */
typedef struct _OesQryCrdUnderlyinInfoRsp {
    /** 查询应答消息头 */
    OesQryRspHeadT      rspHead;
    /** 融资融券可充抵保证金证券及融资融券标的信息数组 */
    OesCrdUnderlyingInfoItemT
                        qryItems[OES_MAX_CRD_UNDERLYING_INFO_ITEM_CNT_PER_PACK];
} OesQryCrdUnderlyingInfoRspT;


/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_CRD_UNDERLYING_INFO_RSP                         \
        {NULLOBJ_OES_QRY_RSP_HEAD}, \
        {{NULLOBJ_OES_CRD_UNDERLYING_INFO_ITEM}}
/* -------------------------           */
...
typedef struct _OesCrdCashPositionItem {
    ...
    /** 资金头寸剩余可融资金额 (单位精确到元后四位, 即1元=10000) */
    int64               availableBalance;
    /** 客户代码 */
    char                custId[OES_CUST_ID_MAX_LEN];

    /** 总计调整金额 (取值可以为负数, 包括红冲蓝补金额及主柜划入金额; 单位精确到元后四位, 即1元 = 10000) */
    int64               totalAdjustAmt;
 ...
 * 查询到的融资融券合约流水信息内容
 */
typedef OesCrdDebtJournalBaseInfoT  OesCrdDebtJournalItemT;
 ...
 */
typedef OesCrdSecurityDebtStatsBaseInfoT                                \
...
typedef struct _OesQryCrdInterestRateRsp {
    ...
    OesCrdInterestRateItemT
                        qryItems[OES_MAX_CRD_INTEREST_RATE_ITEM_CNT_PER_PACK];
...
/* ===================================================================
 * 查询融资融券业务最大可取资金相关结构体定义
 * =================================================================== */

/**
 * 融资融券业务最大可取资金信息内容
 */
typedef struct _OesCrdDrawableBalanceItem {
    /** 客户代码 */
    char                custId[OES_CUST_ID_MAX_LEN];
    /** 资金账户代码 */
    char                cashAcctId[OES_CASH_ACCT_ID_MAX_LEN];
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OES_CRD_DRAWABLE_BALANCE_ITEM       \
        {0}, {0}, 0
/* -------------------------           */
 ...
 * 查询融资融券业务最大可取资金信息请求
 */
typedef struct _OesQryCrdDrawableBalanceReq {
    /** 客户代码, 可选项 */
    char                custId[OES_SECURITY_ID_MAX_LEN];
    /** 资金账户代码, 可选项 */
    char                cashAcctId[OES_CASH_ACCT_ID_MAX_LEN];
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_CRD_DRAWABLE_BALANCE_REQ    \
        {0}, {0}
/* -------------------------           */
 ...
 * 查询融资融券业务最大可取资金信息应答
 */
typedef struct _OesQryCrdDrawableBalanceRsp {
    /** 融资融券业务最大可取资金信息 */
    OesCrdDrawableBalanceItemT
                        drawableBalanceItem;
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_CRD_DRAWABLE_BALANCE_RSP    \
        {NULLOBJ_OES_CRD_DRAWABLE_BALANCE_ITEM}
/* -------------------------           */


/* ===================================================================
 * 查询融资融券担保品可转出的最大数量信息相关结构体定义
 * =================================================================== */

/**
 * 融资融券担保品可转出的最大数量信息内容
 */
typedef struct _OesCrdCollateralTransferOutMaxQtyItem {
    /** 客户代码 */
    char                custId[OES_CUST_ID_MAX_LEN];
    /** 证券代码 */
    char                securityId[OES_SECURITY_ID_MAX_LEN];
    ...
    uint8               __filler[7];
    /** 融资融券担保品可转出的最大数量 */
    int64               collateralTransferOutMaxQty;
} OesCrdCollateralTransferOutMaxQtyItemT;


/* 结构体的初始化值定义 */
#define NULLOBJ_OES_CRD_TRANSFER_OUT_MAX_QTY_ITEM       \
        {0}, {0}, 0, {0}, 0
/* -------------------------           */
 ...
 */
typedef struct _OesQryCrdCollateralTransferOutMaxQtyReq {
    /** 客户代码 */
    char                custId[OES_CUST_ID_MAX_LEN];
    /** 证券代码, 必填项 */
    char                securityId[OES_SECURITY_ID_MAX_LEN];
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_CRD_TRANSFER_OUT_MAX_QTY_REQ    \
        {0}, {0}, 0, {0}
/* -------------------------           */
 ...
 */
typedef struct _OesQryCrdCollateralTransferOutMaxQtyRsp {
    /** 融资融券担保品可转出的最大数量信息 */
    OesCrdCollateralTransferOutMaxQtyItemT
                        collateralTransferOutMaxQtyItem;
} OesQryCrdCollateralTransferOutMaxQtyRspT;


/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_CRD_TRANSFER_OUT_MAX_QTY_RSP    \
        {NULLOBJ_OES_CRD_TRANSFER_OUT_MAX_QTY_ITEM}
/* -------------------------           */
