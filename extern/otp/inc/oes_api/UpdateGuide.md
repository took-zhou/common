...
### 1. API更新概要
  ...
  3. 为异步API添加辅助的连接管理接口 (用于控制是否需要暂停连接重建处理等特殊场景)
       | API                                            | 描述
       | ---------------------------------------------- | ---------------
       | __OesAsyncApi_SetChannelSuspended              | 设置通道的连接管理标志为 '暂停重连', 以暂停该通道的连接重建处理
       | __OesAsyncApi_IsChannelSuspended               | 返回通道的连接管理标志是否为 '暂停重连'
       | __OesAsyncApi_GetChannelSuspendExpirationTime  | 返回通道暂停重连的过期时间
...
### 3. 注意事项
  ...
  1. 沪市全面注册制改革相关注意事项
    - 因综合业务平台发生调整, 需要避免在 9.25 ~ 9.30 期间执行ETF申赎操作, 否则会被拒单 (之前会被暂存到EzSTEP报盘库)
    - 包括主板股票在内的所有股票、存托凭证、基金均支持 ‘本方最优’ 市价订单和 ‘对手方最优’ 市价订单 (之前两种市价订单仅适用于科创板)
    - 对于所有市价订单, 申报时均需设置保护限价, 以表示能够接受的最高买入价或最低卖出价 (若未设置保护限价, 对于有涨跌幅价格的证券, OES将使用涨跌停价格作为该笔申报的保护限价)
  2. 深市全面注册制改革相关注意事项
    - 因存拒单机制发生调整, 超过申报价格范围的订单会被直接拒单 (之前会被暂存在交易主机)


---

OES_0.17.6.7 / 2022-11-23
-----------------------------------
...
### 1. API更新概要
  ...
  2. 为支持将登录密码配置为加密后的密文密码, 而添加如下接口和工具:
     - 增加如下辅助接口
       | API                                   | 描述
       | ------------------------------------- | ---------------
       | OesApi_SetLocalSecretKey              | 设置客户端本地安全密钥 (对当前进程生效, 用于解密密文密码)
       | OesApi_ResetLocalSecretKey            | 重置(清空)客户端本地安全密钥 (对当前进程生效)
       | OesHelper_GenerateCiphertextPassword  | 生成加密后的密文密码字符串

     - 增加生成密文密码的工具: oes_tester 工具中增加生成密文密码的指令 generate-ciphertext-password (pwd) 指令, 以方便客户使用
     - 增加生成密文密码的样例代码: samples/oes_sample_c/11_oes_generate_ciphertext_password_sample.c

### 2. 服务端更新概要

  1. 完善行情价格采集和过滤处理，弱化对行情时间等数据的依赖，以提高容错性
  2. 其他细节完善和优化

### 3. 注意事项

  1. 为支持将登录密码配置为加密后的密文密码, 而扩展配置结构 (SGeneralClientAddrInfoT, SGeneralClientRemoteCfgT) 中 password 字段的长度:
...
### 1. API更新概要
  ...
  2. fix: 消除API接口中隐藏的风险及隐患
     ...
     - 修复API在批量发送委托请求失败并输出错误日志时会存在访问内存越界问题的BUG
  3. 为查询通道增加测试请求接口
    | API                                   | 描述
    | ------------------------------------- | ---------------
    | OesApi_TestQryChannel                 | 发送查询通道的测试请求消息 (同步的请求/应答消息)
    | OesApi_TestQryChannel2                | 发送查询通道的测试请求消息 (同步的请求/应答消息)
  4. 新增辅助的异步API配置项和配置接口
    | API                                   | 描述
    | ------------------------------------- | ---------------
    | OesAsyncApi_SubscribeNothingOnConnect | 连接完成后处理的默认实现 (不订阅任何回报数据)
  5. 查询接口返回的 '融资融券资金头寸信息内容(`OesCrdCashPositionItemT`)' 中:
     - 新增 '总计调整金额(`totalAdjustAmt`)' 字段, 取值可以为负数, 包含红冲蓝补金额及主柜划入金额
  6. 查询接口返回的 '融资融券可充抵保证金证券及融资融券标的信息内容(`OesCrdUnderlyingInfoItemT`)' 中:
     - 新增 '公允价格(`fairPrice`)' 字段
  7. 查询接口返回的 '期权行权指派信息内容(`OesOptExerciseAssignItemT`)' 中:
     - '期权合约名称(`securityName`)' 字段最大长度由C24调整至C56
  8. '有效竞价范围基准价类型 (`eOesAuctionReferPriceTypeT`)' 中:
     - 新增 '买一卖一价 (`OES_AUCTION_REFER_PRICE_TYPE_SSE_BEST`)' 定义, 仅适用于上交所
  9. 新增如下错误码
    | 错误码 | 描述
    | ---- | ---------------
    | 1044 | 数据无变化
...
### 3. 注意事项
  ...
  1. 查询期权行权指派信息接口的应答消息(`OesOptExerciseAssignItemT`) 结构体定义有变动。C/C++ API接口保持兼容, 使用其他编程语言(如Python)封装API接口的客户须留意此变动的影响


---

OES_0.17.6.3_u5 / 2022-11-07
-----------------------------------
