...
==============================================
  ...
  * fix: 修复OesApi_GetFromChannelGroup接口 当指定小于0的下标时，返回值不符合预期的问题
...
==============================================
  ...
  * feat: 在测试工具(oes_tester)中增加生成密文密码的指令 (generate-ciphertext-password), 以方便客户使用
...
==============================================
  ...
  * Merge OES_0.17.6.3_u5
    - fix: 修复 '测试文件路径是否可写函数' 存在BUG，导致初始化日志时可能误判日志路径不可写进而导致API初始化失败的问题
    - fix: 修复当优先使用大页内存时异步API释放大页内存失败的问题
    - fix: 修复API在批量发送委托请求失败并输出错误日志时会存在访问内存越界问题的BUG
    - feat: 新增辅助的异步API接口, 以提供登录后不订阅任何回报数据的快捷方法
      - OesAsyncApi_SubscribeNothingOnConnect, 连接完成后处理的默认实现 (不订阅任何回报数据)

OES_0.17.6.5 / 2022-10-11
==============================================

  * 无

OES_0.17.6.4_u1 / 2022-09-26
==============================================

  * chore: 为查询通道增加测试请求接口 (同步的请求/应答消息)
    - OesApi_TestQryChannel
    - OesApi_TestQryChannel2
  * chore: 增加辅助的异步API接口, 以提供登录后不订阅任何回报数据的快捷方法
    - OesAsyncApi_SubscribeNothingOnConnect, 连接完成后处理的默认实现 (不订阅任何回报数据)
  * chore: 查询接口返回的 资金头寸信息内容(OesCrdCashPositionItemT) 中:
    - 新增 总计调整金额(totalAdjustAmt) 字段, 取值可以为负数, 包含红冲蓝补金额及主柜划入金额
  * chore: 查询接口返回的 融资融券可充抵保证金证券及融资融券标的信息内容(OesCrdUnderlyingInfoItemT) 中:
    - 新增 公允价格(fairPrice) 字段
  * chore: 有效竞价范围基准价类型(eOesAuctionReferPriceTypeT) 中:
    - 新增 买一卖一价(OES_AUCTION_REFER_PRICE_TYPE_SSE_BEST) 定义, 仅适用于上交所
  * chore: 查询接口返回的 期权行权指派信息内容(OesOptExerciseAssignItemT) 中:
  ...
  * chore: 废弃错误码 "1043, 内部撤单", 改为使用错误码 "1151, 内部撤单"
