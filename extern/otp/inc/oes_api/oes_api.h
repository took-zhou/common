 ...
 *          - '融资融券可充抵保证金证券及融资融券标的基础信息 (OesCrdUnderlyingBaseInfoT)' 结构字段变更:
 *              - 新增 公允价格 (fairPrice) 字段
 *          - 为查询通道增加测试请求接口 (同步的请求/应答消息)
 *              - OesApi_TestQryChannel
 *              - OesApi_TestQryChannel2
 ...
 * 查询单条委托信息
 *
 * @note        查询本实例关联环境号(clEnvId)下流水号为clSeqNo的委托信息
 *              特殊的, 当本实例关联环境号(clEnvId)为0时, 返回客户第一条流水号为clSeqNo的委托信息
 *
 * @param       pQryChannel     查询通道的会话信息
 * @param       clSeqNo         委托流水号
...
/**
 * 发送查询通道的测试请求消息
 * 查询通道的测试请求消息为同步的请求/应答消息
 *
 * @param   pQryChannel         查询通道会话信息
 * @param   pTestReqId          测试请求标识符 (C32, 可以为空)
 * @param   testReqIdSize       测试请求标识符的长度
 * @return  TRUE 成功; FALSE 失败
 */
BOOL    OesApi_TestQryChannel(
                OesApiSessionInfoT *pQryChannel,
                const char *pTestReqId,
                int32 testReqIdSize);
 ...
 */
void    OesApi_SetLocalSecretKey(
                const char *pSecretKey);

/**
 * 重置(清空)客户端本地安全密钥 (对当前进程生效)
 */
void    OesApi_ResetLocalSecretKey();
 ...
 *                  小于0, 负的错误号
 */
int32   OesHelper_GenerateCiphertextPassword(
                char *pOutBuf,
                int32 bufSize,
                const char *pUsername,
                const char *pPassword,
