 ...
 *          - 增加辅助的异步API接口, 以提供登录后不订阅任何回报数据的快捷方法
 ...
 *
 * <p> 提示:
 * - 可以在 OnConnect 回调函数中调用该接口来实现不订阅任何回报数据的功能
 ...
 * @retval  <0                  小于0, 处理失败, 异步线程将中止运行
 */
int32   OesAsyncApi_SubscribeNothingOnConnect(
                OesAsyncApiChannelT *pAsyncChannel,
                void *pCallbackParams);
/* -------------------------           */
 ...
 * @note    提示: 委托通道和回报通道需要分别设置
 */
void    __OesAsyncApi_SetChannelSuspended(
                OesAsyncApiChannelT *pAsyncChannel,
                BOOL isSuspended,
 ...
 */
BOOL    __OesAsyncApi_IsChannelSuspended(
                const OesAsyncApiChannelT *pAsyncChannel);
 ...
 *          - 小于等于0表示没有过期时间 (即一直保持暂停重连状态)
 */
int64   __OesAsyncApi_GetChannelSuspendExpirationTime(
                const OesAsyncApiChannelT *pAsyncChannel);
/* -------------------------           */
