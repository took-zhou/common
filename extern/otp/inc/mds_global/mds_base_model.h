 ...
 *          - 调整 '消息来源 (eMdsMsgSourceT)' 枚举类型定义, 针对上交所逐笔合并数据, 增加如下消息来源:
 *              - SSE-VDE-逐笔合并数据 (MDS_MSGSRC_VDE_MERGED_TICK)
 *              - SSE-VDE-逐笔合并数据-逐笔重建 (MDS_MSGSRC_VDE_MERGED_TICK_REBUILD)
 *          - 增加枚举类型定义:
 *              - 新增 '上交所产品状态订单的逐笔委托买卖方向字段取值 (eMdsL2SseStatusOrderSideT)' 枚举类型定义
 *          - 调整逐笔委托/逐笔成交下的 ApplSeqNum 字段类型, 由 int32 调整为 uint32, 以更好的兼容上交所逐笔合并数据
 *          - 微调逐笔委托/逐笔成交字段注释信息, 以兼容上交所逐笔合并数据
 *          - 新增 '逐笔频道心跳消息', 以方便下游系统检查和识别逐笔行情消息是否发生丢失
 *              - 对应的结构体定义 @see MdsTickChannelHeartbeatT
 *              - 行情消息类型定义 @see MDS_MSGTYPE_L2_TICK_CHANNEL_HEARTBEAT
 *              - 可订阅的数据种类 @see MDS_SUB_DATA_TYPE_L2_TICK_CHANNEL_HEARTBEAT
 *
 * @since   2016/02/11
 */
...
#define MDS_MAX_TICK_CHANNEL_NO_SCOPE           (10000)
/** 逐笔数据重传请求每次可重传的最大数据数量 */
#define MDS_MAX_TICK_RESEND_ITEM_COUNT          (1000)
 ...
 * - 'E': ENDTR – 交易结束
 */
typedef enum _eMdsL2SseStatusOrderSide {
...
typedef struct _MdsPriceLevelEntry {
    ...
    int64               OrderQty;               /**< 委托数量 (@note 上交所债券的数量单位为手) */
} MdsPriceLevelEntryT;
...
typedef struct _MdsIndexSnapshotBody {
    ...
    uint64              NumTrades;              /**< 成交笔数 (@note 仅适用于深交所, 上交所指数行情中没有成交笔数信息) */
    uint64              TotalVolumeTraded;      /**< 成交总量 (@note 上交所指数的成交量单位是手, 深交所指数的成交量单位是股) */
    int64               TotalValueTraded;       /**< 成交总金额 (金额单位精确到元后四位, 即: 1元=10000) */

    int64               PrevCloseIdx;           /**< 昨日收盘指数 (单位精确到元后四位, 即: 1元=10000) */
    int64               OpenIdx;                /**< 今开盘指数 (单位精确到元后四位, 即: 1元=10000) */
    int64               HighIdx;                /**< 最高指数 (单位精确到元后四位, 即: 1元=10000) */
    int64               LowIdx;                 /**< 最低指数 (单位精确到元后四位, 即: 1元=10000) */
    int64               LastIdx;                /**< 最新指数 (单位精确到元后四位, 即: 1元=10000) */
    int64               CloseIdx;               /**< 今收盘指数 (单位精确到元后四位, 即: 1元=10000) */

    int32               StockNum;               /**< 统计量指标样本个数 (@note 仅适用于深交所成交量统计指标) */
...
typedef struct _MdsStockSnapshotBody {
    ...
    int32               HighPx;                 /**< 最高价 (价格单位精确到元后四位, 即: 1元=10000) */
    int32               LowPx;                  /**< 最低价 (价格单位精确到元后四位, 即: 1元=10000) */
    int32               TradePx;                /**< 成交价 (最新价. 单位精确到元后四位, 即: 1元=10000) */
    int32               ClosePx;                /**< 今收盘价/期权收盘价 (加权平均收盘价, 适用于上交所行情和深交所债券现券交易产品. 单位精确到元后四位, 即: 1元=10000) */

    union {
        /* 适用于基金和期权产品的字段 */
        struct {
            int32       IOPV;                   /**< 基金份额参考净值/ETF申赎的单位参考净值 (适用于基金. 单位精确到元后四位, 即: 1元=10000) */
            int32       NAV;                    /**< 基金 T-1 日净值 (适用于基金, 上交所Level-2实时行情里面没有该字段. 单位精确到元后四位, 即: 1元=10000) */
            uint64      TotalLongPosition;      /**< 期权合约总持仓量 (适用于期权. 单位为张) */
        };
...
typedef struct _MdsL2StockSnapshotBody {
    ...
    int64               TotalBidQty;            /**< 委托买入总量 (@note 上交所债券的数量单位为手) */
    int64               TotalOfferQty;          /**< 委托卖出总量 (@note 上交所债券的数量单位为手) */
    int32               WeightedAvgBidPx;       /**< 加权平均委买价格 (价格单位精确到元后四位, 即: 1元=10000) */
    int32               WeightedAvgOfferPx;     /**< 加权平均委卖价格 (价格单位精确到元后四位, 即: 1元=10000) */

    union {
        /* 仅适用于上交所的委托价位数相关字段 */
        struct {
            int32       BidPriceLevel;          /**< 买方委托价位数 (实际的委托价位总数, @note 仅适用于上交所) */
            int32       OfferPriceLevel;        /**< 卖方委托价位数 (实际的委托价位总数, @note 仅适用于上交所) */
        };

        /* 仅适用于深交所的债券现券交易相关字段 (为支持新债券业务改造而增加的字段) */
        struct {
            int64       BondAuctionValueTraded; /**< 深交所债券匹配成交的成交总金额 (@note 仅适用于深交所债券现券交易产品. 金额单位精确到元后四位, 即: 1元=10000) */
        };
    };
...
typedef struct _MdsL2StockSnapshotIncremental {
    ...
    int32               HighPx;                 /**< 最高价 (价格单位精确到元后四位, 即: 1元=10000) */
    int32               LowPx;                  /**< 最低价 (价格单位精确到元后四位, 即: 1元=10000) */
    int32               TradePx;                /**< 成交价 (价格单位精确到元后四位, 即: 1元=10000) */
    int32               ClosePx;                /**< 今收盘价/期权收盘价 (价格单位精确到元后四位, 即: 1元=10000) */
    int32               IOPV;                   /**< 基金份额参考净值/ETF申赎的单位参考净值 (适用于基金. 单位精确到元后四位, 即: 1元=10000) */
...
typedef struct _MdsL2BestOrdersSnapshotBody {
    ...
    int32               BestBidPrice;           /**< 最优申买价 (价格单位精确到元后四位, 即: 1元=10000) */
    int32               BestOfferPrice;         /**< 最优申卖价 (价格单位精确到元后四位, 即: 1元=10000) */
...
typedef struct _MdsL2Order {
     ...
     * - 深交所为逐笔成交+逐笔委托统一编号
     * - 上交所债券行情和逐笔合并数据为逐笔成交+逐笔委托统一编号 (TickIndex/BizIndex)
     * - 上交所老版竞价行情(非逐笔合并数据)为逐笔委托独立编号 (OrderIndex)
     */
    uint32              ApplSeqNum;
 ...
 */
typedef struct _MdsTickChannelHeartbeat {
    uint8               exchId;                 /**< 交易所代码(沪/深) @see eMdsExchangeIdT */
    uint8               __origMdSource;         /**< 原始行情数据来源 @see eMdsMsgSourceT */
    uint8               __filler1[6];           /**< 按64位对齐的填充域 */

    uint16              ChannelNo;              /**< 频道代码 [0..9999] */
    uint16              __filler2;              /**< 按64位对齐的填充域 */
    uint32              ApplLastSeqNum;         /**< 最后一条逐笔数据的记录号
    ...
    STimespec32T        __origNetTime;
#endif
} MdsTickChannelHeartbeatT;


/* 结构体初始化值的尾部填充字段定义 */
#if defined (_MDS_ENABLE_LATENCY_STATS) || defined (_MDS_ENABLE_ORIG_NET_TIMESTAMP)
# define    __NULLOBJ_MDS_TICK_CHANNEL_HEARTBEAT_TAILER                 \
            , {0, 0}
#else
# define    __NULLOBJ_MDS_TICK_CHANNEL_HEARTBEAT_TAILER
#endif
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_TICK_CHANNEL_HEARTBEAT                              \
        0, 0, {0}, \
        0, 0, 0, \
        0, {0}, {0} \
        __NULLOBJ_MDS_TICK_CHANNEL_HEARTBEAT_TAILER
/* -------------------------           */
...
typedef struct _MdsStockStaticInfo {
    ...
    uint8               isNoProfit;             /**< 是否尚未盈利 (0:已盈利, 1:未盈利) */
    uint8               isWeightedVotingRights; /**< 是否存在表决权差异 (0:无差异, 1:有差异) */
    uint8               isVie;                  /**< 是否具有协议控制架构 (0:没有协议控制架构, 1:具有协议控制架构. 仅适用于深交所, 对于上交所该字段固定为0) */
    ...
    union {
        ...
        int32           limitUpPrice;           /**< 涨停价 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
    };
    union {
        int32           lowerLimitPrice;        /**< 跌停价 (单位精确到元后四位, 即: 1元=10000) */
        int32           limitDownPrice;         /**< 跌停价 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
    };
    int32               priceTick;              /**< 价格档位 (单位精确到元后四位, 即: 1元=10000) */
    int32               prevClose;              /**< 前收盘价 (单位精确到元后四位, 即: 1元=10000) */

    union {
        int32           lmtBuyMaxQty;           /**< 单笔限价买委托数量上限 */
        int32           buyOrdMaxQty;           /**< 单笔限价买委托数量上限 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
    };
    union {
        int32           lmtBuyMinQty;           /**< 单笔限价买委托数量下限 */
        int32           buyOrdMinQty;           /**< 单笔限价买委托数量下限 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
    };
    union {
        int32           lmtBuyQtyUnit;          /**< 单笔限价买入单位 */
        int32           buyQtyUnit;             /**< 单笔限价买入单位 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
    };
    ...
    union {
        ...
        int32           sellOrdMaxQty;          /**< 单笔限价卖委托数量上限 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
    };
    union {
        int32           lmtSellMinQty;          /**< 单笔限价卖委托数量下限 */
        int32           sellOrdMinQty;          /**< 单笔限价卖委托数量下限 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
    };
    union {
        int32           lmtSellQtyUnit;         /**< 单笔限价卖出单位 */
        int32           sellQtyUnit;            /**< 单笔限价卖出单位 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
    };
    ...
    union {
        int64           parValue;               /**< 面值 (单位精确到元后四位, 即: 1元=10000) */
        int64           parPrice;               /**< 面值 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
    };
