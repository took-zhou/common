...
#ifndef _YTP_TRADER_STRUCT_TYPE_H_
#define _YTP_TRADER_STRUCT_TYPE_H_

#include <cstdint>

struct YtpLoginLogoutStruct {
  char user_id[10];
  char password[10];
};

struct YtpOrderInfoStruct {
...
struct YtpTransactionCostField {
  char exchange_id[10];
  char instrument_id[32];
  char user_id[10];
};

struct YtpCommissionInfo {
  char exchange_id[10];
  char instrument_id[32];
  char user_id[10];
  int32_t prid;
