#ifndef _MTP_TRADER_STRUCT_TYPE_H_
#define _MTP_TRADER_STRUCT_TYPE_H_
...
struct MtpTransactionCostField {
  char exchange_id[10];
  char instrument_id[32];
  char user_id[10];
};

struct MtpCommissionInfo {
  char exchange_id[10];
  char instrument_id[32];
  char user_id[10];
  int32_t prid;
