...
#ifndef _FTP_TRADER_STRUCT_TYPE_H_
#define _FTP_TRADER_STRUCT_TYPE_H_

#include <cstdint>

struct FtpLoginLogoutStruct {
  char user_id[10];
  char password[10];
};

struct FtpOrderInfoStruct {
...
struct FtpTransactionCostField {
  char exchange_id[10];
  char instrument_id[32];
  char user_id[10];
};

struct FtpCommissionInfo {
  char exchange_id[10];
  char instrument_id[32];
  char user_id[10];
  int32_t prid;
