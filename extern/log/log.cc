...
ring_log::ring_log()
    : _buff_cnt(3), _curr_buf(NULL), _prst_buf(NULL), _fp(NULL), _log_cnt(0), _env_ok(false), _level(INFO), _lst_lts(0), _tm() {
...
void ring_log::persist() {
  while (true) {
    ...
    if (!decis_file(year, mon, day)) continue;
    // write
...
void ring_log::try_append(const char* lvl, const char* format, ...) {
  ...
  if (_lst_lts && curr_sec - _lst_lts < RELOG_THRESOLD) return;

  char log_line[LOG_LEN_LIMIT];
  // int prev_len = snprintf(log_line, LOG_LEN_LIMIT, "%s[%d-%02d-%02d %02d:%02d:%02d.%03d]", lvl, _tm.year, _tm.mon, _tm.day, _tm.hour,
  // _tm.min, _tm.sec, ms);
